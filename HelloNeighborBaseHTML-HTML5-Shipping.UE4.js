...
var Module = {
	...
	UE4_indexedDBVersion: 202601090325, // Bump this number to invalidate existing IDB storages in browsers.
};
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('HelloNeighborBaseHTML-HTML5-Shipping.wasm'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('HelloNeighborBaseHTML-HTML5-Shipping.js'), 'blob').then(function(data) {
